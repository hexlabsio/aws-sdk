import { Request, CloudWatchEvents as AWSCloudWatchEvents } from 'aws-sdk';
// @ts-ignore
type ReturnTypeFrom<K extends keyof AWSCloudWatchEvents> = AWSCloudWatchEvents[K] extends (...args: any) => Request<infer R, any> ? R : never;
// @ts-ignore
type FunctionTypeFrom<K extends keyof AWSCloudWatchEvents> = AWSCloudWatchEvents[K] extends {
  (param: infer P, callback: (...args: any) => void): Request<infer R, any>;
  (callback: (...args: any) => void): any;
} ? (params?: P) => Promise<R> : AWSCloudWatchEvents[K] extends {
  (callback: (...args: any) => void): Request<infer R, any>;
} ? () => Promise<R> : never;
// @ts-ignore
type ParamsFrom<K extends keyof AWSCloudWatchEvents, Extras> = AWSCloudWatchEvents[K] extends {
  (param: infer P, callback: (...args: any) => void): Request<infer R, any>;
  (callback: (...args: any) => void): any;
} ? P & Extras: never;
// @ts-ignore
type RawParamsFrom<K extends keyof AWSCloudWatchEvents> = AWSCloudWatchEvents[K] extends {
  (param: infer P, callback: (...args: any) => void): Request<infer R, any>;
  (callback: (...args: any) => void): any;
} ? P: never;
 
export class CloudWatchEvents {
  private constructor(private readonly client: AWSCloudWatchEvents) {}
  ...
  static fromClient(client: AWSCloudWatchEvents): CloudWatchEvents {
    return new CloudWatchEvents(client) as any;
  }
  
  static client(options?: AWSCloudWatchEvents.Types.ClientConfiguration): CloudWatchEvents {
    return new CloudWatchEvents(new AWSCloudWatchEvents(options)) as any;
  }
}  
